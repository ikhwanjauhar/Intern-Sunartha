	...
	public class ARPaymentEntry : ARDataEntryGraph<ARPaymentEntry, ARPayment>
	{
		...
		protected virtual void ARPayment_CustomerID_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
				 ...
				 && arsetup.Current.AutoLoadMaxDocs > 0)
			{
				...
				{
					string message = PXAccess.FeatureInstalled<FeaturesSet.paymentsByLines>() && PXAccess.FeatureInstalled<FeaturesSet.construction>()
		...
		protected virtual void ARAdjust_Selected_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			*/
			...
			applyARAdjust(adj, payment.CuryUnappliedBal.Value, checkBalance, adj.Selected.Value);
		}
		#endregion
		...
		protected virtual void ARAdjust_AdjdRefNbr_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			if (PXAccess.FeatureInstalled<FeaturesSet.paymentsByLines>() && PXAccess.FeatureInstalled<FeaturesSet.construction>())
			{
		...
		public class PaymentTransaction : PaymentTransactionAcceptFormGraph<ARPaymentEntry, ARPayment>
		{
			...
			protected override void RowSelected(Events.RowSelected<ARPayment> e)
			{
				...
				bool canAuthorize = !docOnHold && docTypePayment && !(extTranState.IsPreAuthorized || extTranState.IsCaptured);
				canAuthorize = canAuthorize && (UseAcceptHostedForm == false || cache.GetStatus(doc) != PXEntryStatus.Inserted);
				bool canCapture = !docOnHold && docTypePayment && !extTranState.IsCaptured;
				canCapture = canCapture && (UseAcceptHostedForm == false || (cache.GetStatus(doc) != PXEntryStatus.Inserted && !extTranState.IsOpenForReview));
				...
				bool canValidate = !docOnHold && cache.GetStatus(doc) != PXEntryStatus.Inserted 
					&& (canCapture || canAuthorize || extTranState.IsOpenForReview || doc.PMInstanceID == PaymentTranExtConstants.NewPaymentProfile);
			...
			{
				graph.Document.View.SetAnswer(PaymentRefAttribute.PaymentRef_AskKeyUpdateLastRefNbr, WebDialogResult.No);
		...
		protected virtual bool VerifyAdjFinPeriodID(ARPayment doc, string newValue, out string errMsg)
		{
			errMsg = null;
			ExternalTransactionState state = ExternalTranHelper.GetActiveTransactionState(this, ExternalTran);
			if ((doc.Released == true || state.IsSettlementDue) &&
