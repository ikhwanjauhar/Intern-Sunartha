...
namespace PX.Objects.AR
{
	...
	public class ARInvoiceEntry : ARDataEntryGraph<ARInvoiceEntry, ARInvoice>, PXImportAttribute.IPXPrepareItems, IGraphWithInitialization
	{
		...
		[PXDefault(typeof(Where2<FeatureInstalled<FeaturesSet.paymentsByLines>,
			And2<FeatureInstalled<FeaturesSet.construction>,
		...
		[PXFormula(typeof(Selector<ARTran.inventoryID, InventoryItem.stkItem>))]
		[PXMergeAttributes(Method = MergeMethod.Append)]
		public void ARTran_IsStockItem_CacheAttached(PXCache sender)
		{
		}
		...
		public virtual void ReverseInvoiceProc(ARRegister doc)
		{
			...
			foreach (PXResult<ARInvoice, CurrencyInfo, Terms, Customer> res in ARInvoice_CurrencyInfo_Terms_Customer.Select(this, doc.DocType, doc.RefNbr))
			{
				...
				invoice.Hold = (ARSetup.Current.HoldEntry ?? false) || IsApprovalRequired(invoice, Document.Cache);
		...
		protected virtual void ARInvoice_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			bool isDocumentOnHold = doc.Hold == true;
			bool isDocumentScheduled = doc.Scheduled == true;
			bool isDocumentVoided = doc.Voided == true;
			bool isDocumentRejected = doc.Rejected == true;
			bool invoiceUnreleased = (isDocumentInvoice || isDocumentDebitMemo) && doc.Released == false && doc.CustomerID != null;
			...
			autoApply.SetEnabled(invoiceUnreleased);
			loadDocuments.SetEnabled(invoiceUnreleased);
			...
			Adjustments.Cache.AllowDelete = invoiceUnreleased;
			Adjustments.Cache.AllowUpdate = 
				Transactions.Cache.AllowUpdate && !isRetainageCreditMemo 
			...
			if (ARSetup.Current != null)
			{
				...
				if ((bool)doc.Released || (bool)doc.Voided || isDocumentRejectedOrPendingApproval || isDocumentApprovedBalanced)
				{
					...
					this.salesPerTrans.Cache.AllowUpdate = !isCommnPeriodClosed && !isDocumentRejectedOrPendingApproval && !isDocumentApprovedBalanced;
			...
			bool isTaxZoneIDEnabled =
				...
				&& !isDocumentRejectedOrPendingApproval
				&& !isDocumentApprovedBalanced;
			PXUIFieldAttribute.SetEnabled<ARInvoice.taxZoneID>(cache, e.Row, isTaxZoneIDEnabled);

			bool isAvalaraCustomerUsageTypeEnabled =
				doc.ProformaExists != true
				&& doc.Released != true
				&& !isDocumentRejectedOrPendingApproval
				&& !isDocumentApprovedBalanced;
			PXUIFieldAttribute.SetEnabled<ARInvoice.avalaraCustomerUsageType>(cache, e.Row,isAvalaraCustomerUsageTypeEnabled);
			...
			PXUIFieldAttribute.SetVisible<ARTran.lineNbr>(Transactions.Cache, null, isPaymentsByLinesAllowed);
			...
			if (isPaymentsByLinesAllowed)
			{
				...
				if (!isDocumentReleased && !UnattendedMode)
				{
			...
			PXUIFieldAttribute.SetEnabled<ARTran.tranCost>(Transactions.Cache, null, isMigrationMode);
			PXUIFieldAttribute.SetVisible<ARTran.tranCost>(Transactions.Cache, null, isMigrationMode);
			PXUIFieldAttribute.SetVisibility<ARTran.tranCost>(Transactions.Cache, null, isMigrationMode ? PXUIVisibility.Visible : PXUIVisibility.Invisible);
		...
		private static void CalculateAccruedCost(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			decimal? accruedCost = (decimal?)PXFormulaAttribute.Evaluate<ARTran.accruedCost>(sender, e.Row);
			sender.SetValueExt<ARTran.accruedCost>(e.Row, accruedCost);
		}
		...
		public virtual void AddTaxesAndApplications(PendingPPDCreditMemoApp doc)
		{
			...
			foreach (PXResult<ARTaxTran, Tax> res in taxes)
			{
				...
				artaxNew.CuryTaxAmtSumm += CuryTaxAmtSumm;
				...
				if (tax.TaxCalcLevel == CSTaxCalcLevel.Inclusive 
					&& Document.Current.TaxCalcMode != TaxCalculationMode.Net 
					|| Document.Current.TaxCalcMode == TaxCalculationMode.Gross)
				{
