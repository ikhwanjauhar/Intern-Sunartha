		...
		public static void ProcessDoc(ARPaymentEntry graph, ARStatementCycle cycle, ARAutoApplyParameters filter)
		{
			...
			foreach (Customer customer in GetCustomersForAutoApplication(graph, cycle, filter))
			{
				...
				if (arInvoiceList.Count > 0)
				{
					...
					{
						List<ARInvoice> invoicesForPayment = arInvoiceList.Where(inv => DateTime.Compare((DateTime)inv.DocDate, (DateTime)filter.ApplicationDate) <= 0).ToList();
						ApplyPayment(graph, filter, payment, invoicesForPayment, toRelease, out warningRefNbr);
