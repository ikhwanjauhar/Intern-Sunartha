	...
	public class ARReleaseProcess : PXGraph<ARReleaseProcess>
	{
			...
			out PMRegister pmDoc)
		{
			...
			if (doc.Released != true)
			{
				...
				bool calculateLineBalances = doc.PaymentsByLinesAllowed == true || 
				...
				foreach (var group in docLinesWithData.AsEnumerable().GroupBy(row => (ARTran)row, arTranComparer))
				{
									...
									And<INTran.sOOrderType, Equal<Current<ARTran.sOOrderType>>, And<INTran.sOOrderNbr, Equal<Current<ARTran.sOOrderNbr>>, And<INTran.sOOrderLineNbr, Equal<Current<ARTran.sOOrderLineNbr>>>>>>>>.SelectMultiBound(this, new object[] { n }))
								{
									...
									if (intran.UpdateShippedNotInvoiced == true)
									{
										...
										foreach (IN.INTranCost trancost in trancosts)
										{
											...
											ReasonCode reasoncode = INTran.FK.ReasonCode.FindParent(this, intran);
											IN.INSite site = IN.INSite.PK.Find(this, intran.SiteID);
											...
											if (trancost != null && trancost.COGSAcctID != null && intran != null)
											{
												...
												tranFromIN.AccountID = intran.COGSAcctID ?? IN.INReleaseProcess.GetAccountDefaults<IN.INPostClass.cOGSAcctID>(this, (IN.InventoryItem)itemPostClassRes, site, (IN.INPostClass)itemPostClassRes, intran);
												tranFromIN.SubID = intran.COGSSubID ?? IN.INReleaseProcess.GetAccountDefaults<IN.INPostClass.cOGSSubID>(this, (IN.InventoryItem)itemPostClassRes, site, (IN.INPostClass)itemPostClassRes, intran);
												...
												if (reasoncode?.Usage == ReasonCodeUsages.Issue)
												{
													...
													tranFromIN.SubID = IN.INReleaseProcess.GetReasonCodeSubID(this, reasoncode, (IN.InventoryItem)itemPostClassRes, site, (IN.INPostClass)itemPostClassRes);
												}
												else
												{
						...
						!isMultipleInstallmentInvoice)
					{
						...
						if (ardoc.IsChildRetainageDocument())
						{
							...
							if (origRetainageTran != null)
							{
								origRetainageTran.CuryRetainageBal -= n.CuryOrigTranAmt * ardoc.SignAmount;
								origRetainageTran.RetainageBal -= n.OrigTranAmt * ardoc.SignAmount;
				...
				foreach (PXResult<ARTaxTran, Tax, Account> rs in ARTaxTran_TranType_RefNbr.Select(ardoc.DocType, ardoc.RefNbr))
				{
					...
					docInclTaxDiscrepancy += (x.CuryTaxAmt ?? 0.0m) - (x.CuryTaxAmtSumm ?? 0.0m);
				...
				if (PXAccess.FeatureInstalled<FeaturesSet.netGrossEntryMode>() && docInclTaxDiscrepancy != 0 && ardoc.TaxCalcMode == TaxCalculationMode.Gross)
				{
					...
					Currency currency = PXSelect<Currency, Where<Currency.curyID, Equal<Required<Currency.curyID>>>>.SelectSingleBound(this, new object[] { }, ardoc.CuryID);
					GLTran diffTran = new GLTran();
					diffTran.SummPost = this.SummPost;
					diffTran.BranchID = ardoc.BranchID;
					diffTran.CuryInfoID = new_info.CuryInfoID;
					...
					diffTran.TranDesc = "DOC INCL TAX DISCREPANCY";
					...
					InsertInvoiceTransaction(je, diffTran, new GLTranInsertionContext { ARRegisterRecord = doc });
				}
			...
			Func<decimal, decimal> round)
		{
			...
			if (applyNetRevenue)
			{
				if (!postedPPD && 
					lineTax?.TaxID != null && 
					(salesTax?.TaxCalcLevel == CSTaxCalcLevel.Inclusive || document.TaxCalcMode == TX.TaxCalculationMode.Gross))
			{
		...
		{
			var docList = startFrom;
			while (docList.Any())
			{
				var currentList = docList.ToList();
				docList = new PXResultset<ARRegister>();
				foreach (ARRegister doc in currentList)
				{
					var adjustments =
						PXSelectJoinGroupBy<ARRegister,
					...
					{
						if (!documents.Any(_ => ((ARRegister)_).DocType == ((ARRegister)adj).DocType && ((ARRegister)_).RefNbr == ((ARRegister)adj).RefNbr))
						{
							docList.Add(adj);
							documents.Add(adj);
						}
