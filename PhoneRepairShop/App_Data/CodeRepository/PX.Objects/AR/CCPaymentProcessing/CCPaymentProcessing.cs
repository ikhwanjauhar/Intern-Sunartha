...
namespace PX.Objects.AR.CCPaymentProcessing
{
	public class CCPaymentProcessing
	{
		...
		public virtual TranOperationResult Authorize(ICCPayment aPmtInfo, bool aCapture)
		{
			...
			IsValidForProcessing(pmInstance, customer);
		...
		private void ValidateRelativelyRefTran(ICCPayment payment, TranRecordData recordData, CCProcTran refProcTran)
		{
			...
			if ((docType != pDocType || refNbr != pRefNbr
				|| origDocType != pOrigDocType || origRefNbr != pOrigRefNbr)
		...
		protected virtual void IsCCValidForProcessing(int? aPMInstanceID)
		{
			Customer customer;
			CustomerPaymentMethod pmInstance = this.findPMInstance(aPMInstanceID, out customer);
			if (pmInstance == null)
		...
		protected virtual void IsValidForProcessing(CustomerPaymentMethod pmInstance, Customer customer)
		{
			if (pmInstance == null)
			{
