		...
		protected virtual void _(Events.FieldSelecting<PMProformaProgressLine, PMProformaProgressLine.currentInvoicedPct> e)
		{
			if (e.Row != null)
			{
				...
				if (budget != null)
				{
					...
					if (budget.CuryRevisedAmount.GetValueOrDefault() != 0)
					{
						result = Math.Round(100m * invoicedAmount / budget.CuryRevisedAmount.Value, precision);
					}
					PXFieldState fieldState = PXDecimalState.CreateInstance(result, precision, nameof(PMProformaProgressLine.CurrentInvoicedPct), false, 0, Decimal.MinValue, Decimal.MaxValue);
					e.ReturnState = fieldState;
				}
			}
		}
