	...
	public class CRMassMailMaint : PXGraph<CRMassMailMaint, CRMassMail>
	{
		...
		public virtual IEnumerable previewMail(PXAdapter a)
		{
				...
				== WebDialogResult.OK)
			{
				...
				AddSendedMessages(massMail, SendEmailsToRecipient(massMail, recipient));
		...
		private void SendMails()
		{
			...
			var mails = EnumerateRecipientsForSending().ToList();
			...
			PXLongOperation.StartOperation(this, () => ProcessMassMailEmails(MassMails.Current, mails));
		}

		private static void ProcessMassMailEmails(
			CRMassMail massMail,
			IEnumerable<Recipient> recipients)
		{
			var processor = PXGraph.CreateInstance<CRMassMailMaint>();

			var entityHelper = new EntityHelper(processor);

			foreach (Recipient recipient in recipients)
			{
				var entity = recipient.Entity;
				var cache = processor.Caches[entity.GetType()];
				cache.SetStatus(entity, PXEntryStatus.Notchanged);

				Guid? refNoteId = entityHelper.GetEntityNoteID(entity, true);

				IEnumerable<CRSMEmail> messages;
				try
				{
					messages = processor.SendEmailsToRecipient(massMail, recipient, refNoteId);
				}
				catch (Exception e)
				{
		...
		private IEnumerable<CRSMEmail> SendEmailsToRecipient(
			CRMassMail massMail,
			Recipient recipient,
			Guid? linkRefNoteId = null)
		{
			var sender = TemplateNotificationGenerator.Create(recipient.Contact);
			...
			sender.BodyFormat = recipient.Format;
			sender.AttachmentsID = massMail.NoteID;

			if (linkRefNoteId is Guid refNoteId)
			{
				sender.RefNoteID = refNoteId;
				sender.BAccountID = recipient.Contact.BAccountID;
				sender.ContactID = recipient.Contact.ContactID;
			}
						...
						if (list.Selected == true)
						{
							...
							foreach (PXResult row in rows)
							{
								...
								if (address != null && (address = address.Trim()) != string.Empty && !mailKeys.Contains(address))
								{
									...
									yield return new Recipient(list, contact, subscription?.Format ?? NotificationFormat.Html);
								}
							}
						}
					break;
					...
					foreach (CRCampaign list in Campaigns.Select())
						if (list.Selected == true)
						{
							...
							foreach (PXResult<Contact, CRCampaignMembers> item in select.Select(list.CampaignID))
							{
								...
								{
									mailKeys.Add(address);
									yield return new Recipient(list, contact, NotificationFormat.Html);
								}
							}
						}
					break;
				case CRMassMailSourcesAttribute.Lead:
					foreach (Contact contact in Leads.Select())
						if (contact.Selected == true)
						{
							...
							{
								mailKeys.Add(address);
								yield return new Recipient(contact, NotificationFormat.Html);
							}
						}
					break;
			}
