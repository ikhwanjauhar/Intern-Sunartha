...
namespace PX.Objects.IN
{
		...
		where TWMSHeader : WMSHeader, IBqlTable, new()
	{
			...
			where TArguments : WMSWaitArguments<TData>
		{
			...
			void safeMethod(object[] arguments)
			{
				...
				{
					var status = ex is PXRedirectRequiredException 
	...
	public class NonStockKitSpecHelper
	{
		...
		protected decimal ConvertToBaseQty(int? inventoryID, string uom, decimal? qty) => INUnitAttribute.ConvertToBase(_graph.Caches.Cast<PXCache>().First(), inventoryID, uom, qty ?? 0, INPrecision.NOROUND);
	}
}