			...
			foreach (InventoryItem e in (new PXCancel<InventoryItem>(this, "Cancel")).Press(a))
			{
				if (Item.Cache.GetStatus(e) == PXEntryStatus.Inserted)
				{
					InventoryItem e1 = PXSelect<InventoryItem,
						Where<InventoryItem.inventoryCD, Equal<Required<InventoryItem.inventoryCD>>, And<InventoryItem.stkItem, NotEqual<Required<InventoryItem.stkItem>>>>>
						.Select(this, e.InventoryCD, IsStockItemFlag);
					if (e1 != null)
					{
						Item.Cache.RaiseExceptionHandling<InventoryItem.inventoryCD>(e, e.InventoryCD,
							new PXSetPropertyException(IsStockItemFlag ? Messages.NonStockItemExists : Messages.StockItemExists));
					}
				}
				yield return e;
			}
