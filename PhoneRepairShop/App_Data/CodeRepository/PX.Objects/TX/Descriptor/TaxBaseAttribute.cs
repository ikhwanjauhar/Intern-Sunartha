...
namespace PX.Objects.TX
{
													 ...
													 IComparable
	{
		...
		{
			if (_isTaxCalcModeEnabled && (taxToAdjust.TaxCalcType == CSTaxCalcType.Item || CalcGrossOnDocumentLevel) && taxToAdjust.TaxCalcLevel != CSTaxCalcLevel.CalcOnItemAmtPlusTaxAmt)
			{
		...
		protected IEnumerable<decimal> GetInclusiveTaxesRates(PXCache sender, object row)
		{
			var rates = new List<decimal>();

			var taxes = SelectInclusiveTaxes(sender.Graph, row);
			taxes.ForEach(delegate (object a)
			{
				decimal taxRate = (decimal?)sender.Graph.Caches[typeof(TaxRev)].GetValue<TaxRev.taxRate>(((PXResult)a)[typeof(TaxRev)]) ?? 0.0m;
				decimal multiplier = ((Tax)(((PXResult)a)[typeof(Tax)])).ReverseTax == true ? Decimal.MinusOne : Decimal.One;
				rates.Add(taxRate * multiplier);
			});
			return rates;
		}
		...
		protected virtual void TaxSetLineDefault(PXCache sender, object taxrow, object row)
		{
			...
			var inclusiveRates = GetInclusiveTaxesRates(sender, row);

			decimal inclRate = inclusiveRates.Sum();
			...
			switch (tax.TaxCalcLevel)
			{
				case CSTaxCalcLevel.Inclusive:
					curyTaxableAmt = curyTranAmt / (1 + inclusiveRates.Sum() / 100);
					curyTaxAmt = PXDBCurrencyAttribute.RoundCury(cache, taxDetail, curyTaxableAmt * calculatedTaxRate, Precision);
					curyInclTaxAmt = 0m;

					inclusiveRates.ForEach(delegate (decimal rate)
					{
						curyInclTaxAmt += PXDBCurrencyAttribute.RoundCury(cache, taxDetail, curyTaxableAmt * rate / 100m, Precision);
					});
		...
		protected virtual TaxDetail CalculateTaxSum(PXCache sender, object taxrow, object row)
		{
			...
			else if (CalcGrossOnDocumentLevel && _isTaxCalcModeEnabled && GetTaxCalcMode(sender.Graph) == TaxCalculationMode.Gross)
			{
			...
			TaxRev taxrev)
		{
			...
			decimal curyTaxDiscountAmt = curyTaxableDiscountAmt * (decimal)taxrev.TaxRate / 100;
