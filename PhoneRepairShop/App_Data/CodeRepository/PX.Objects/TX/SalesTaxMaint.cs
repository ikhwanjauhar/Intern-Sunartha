...
namespace PX.Objects.TX
{
	...
	{		
				...
				case CSTaxType.Sales:
					...
					else
					{
						foreach (TaxBucket bucket in
							PXSelectReadonly<TaxBucket,
							Where<TaxBucket.vendorID, Equal<Required<TaxBucket.vendorID>>,
								And<Where<TaxBucket.bucketType, Equal<Required<TaxBucket.bucketType>>,
									Or<TaxBucket.bucketType, Equal<Required<TaxBucket.bucketType>>>>>>>
							.Select(this, tax.TaxVendorID, CSTaxBucketType.Sales, CSTaxBucketType.Purchase))
						{
							allowedValues.Add((int)bucket.BucketID);
							allowedLabels.Add(bucket.Name);
						}
		...
		protected virtual void Tax_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			PXUIFieldAttribute.SetEnabled<Tax.reportExpenseToSingleAccount>(cache, tax, isDeductible);

			bool expenseAccountIsApplicable = isUse || isSales || requireExpenseAccount;

			PXUIFieldAttribute.SetEnabled<Tax.expenseAccountID>(cache, tax, expenseAccountIsApplicable);
			PXUIFieldAttribute.SetEnabled<Tax.expenseSubID>(cache, tax, expenseAccountIsApplicable);
			PXDefaultAttribute.SetPersistingCheck<Tax.expenseAccountID>(cache, tax, requireExpenseAccount ? PXPersistingCheck.NullOrBlank : PXPersistingCheck.Nothing);
			PXDefaultAttribute.SetPersistingCheck<Tax.expenseSubID>(cache, tax, requireExpenseAccount ? PXPersistingCheck.NullOrBlank : PXPersistingCheck.Nothing);
			PXUIFieldAttribute.SetRequired<Tax.expenseAccountID>(cache, requireExpenseAccount);
			PXUIFieldAttribute.SetRequired<Tax.expenseSubID>(cache, requireExpenseAccount);
		...
		{
			if (tax.ReportExpenseToSingleAccount != true) return false;
			else if (tax.TaxType == CSTaxType.Use || tax.DeductibleVAT == true) return true;
			else
			{
		...
		public SalesTaxMaint()
		{
			...
			PopulateBucketList(new TX.Tax { TaxType = TX.CSTaxType.Sales });
		}
