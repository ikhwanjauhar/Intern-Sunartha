    ...
    public partial class CABankTransactionsMaint : PXGraph<CABankTransactionsMaint>
	{
		...
		public partial class Filter : IBqlTable
		{
			...
			[CashAccount(null, typeof(Search<
			    ...
			    Where<Match<Current<AccessInfo.userName>>>>), IsKey = true)]
	    ...
	    protected virtual IEnumerable expenseReceiptDetailMatches()
	    {
	        ...
	        IList<CABankTranExpenseDetailMatch> matches = this.FindExpenseReceiptDetailMatches(detail, this.matchSettings.Current);
        ...
        protected virtual void EnableTranFields(PXCache sender, CABankTran row)
		{
		    ...
		    if (match != null)
		    {
			    ...
			    matchedToReceipt = IsMatchedToExpenseReceipt(match);
				matchedToInv = !String.IsNullOrEmpty(match.DocRefNbr) && !matchedToReceipt;
		...
		#region MatchSettings Events

        protected virtual void _(Events.RowSelected<MatchSettings> e)
	    {
	        if (e.Row == null)
	            return;
			...
			PXUIFieldAttribute.SetVisible<MatchSettings.curyDiffThreshold>(e.Cache, null, isCorpCard);
		    PXUIFieldAttribute.SetVisible<MatchSettings.amountWeight>(e.Cache, null, isCorpCard);
		    PXUIFieldAttribute.SetVisible<MatchSettings.ratioInRelevanceCalculationLabel>(e.Cache, null, isCorpCard);
		}
		...
		protected virtual List<CABankTran> DoDocumentMatchInner<TMatchRow>(
			...
			Func<CABankTran, MatchSettings, decimal, TMatchRow[], IEnumerable> findDetailMatching)
			where TMatchRow: CABankTranDocumentMatch
		{
			...
			foreach (object en in aRows)
			{
				...
				findDetailMatching(iDet, this.matchSettings.Current, Decimal.Zero, bestMatches);
		...
		public virtual void DoExpenseReceiptMatch(PXCache aUpdateCache, IEnumerable<CABankTran> aRows)
		{
			...
			foreach (CABankTran iRow in aRows)
			{
				if (iRow.DocumentMatched == false && iRow.CreateDocument == false && iRow.CountExpenseReceiptDetailMatches > 0)
				{
					...
					this.FindExpenseReceiptDetailMatches(iRow, this.matchSettings.Current);
				}
			}
		}
	    ...
	    protected virtual IList<CABankTranExpenseDetailMatch> FindExpenseReceiptDetailMatches(CABankTran detail, MatchSettings settings)
	    {
	        decimal relevanceTreshold = skipLowRelevance ? RelevanceTreshold : Decimal.Zero;
			...
			if (aDetail.DrCr == CA.CADrCr.CACredit && !clearingAccount)
			{
				foreach (PXResult<Light.APInvoice, Light.BAccount, CABankTranMatch, Light.APAdjust, Light.APPayment, Light.CABankTranAdjustment> it in
					PXSelectJoin<
					    ...
					    .Select(this, aDetail.TranType, aDetail.CuryID, amount, aDetail.TranDate))
				{
	    ...
	    public virtual decimal EvaluateMatching(CABankTran aDetail, CABankTranExpenseDetailMatch matchRow, MatchSettings aSettings)
	    {
		...
		public virtual decimal CompareExpenseReceiptAmount(CABankTran bankTran, CABankTranExpenseDetailMatch receipt, MatchSettings settings)
		{
	    ...
	    public virtual decimal CompareRefNbr(CABankTran aDetail, CABankTranExpenseDetailMatch aTran, bool looseCompare)
	    {
	        return StatementsMatchingProto.CompareRefNbr(this, aDetail, aTran.ExtRefNbr, looseCompare);
	    }
