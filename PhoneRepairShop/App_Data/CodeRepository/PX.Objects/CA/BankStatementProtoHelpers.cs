...
namespace PX.Objects.CA.BankStatementProtoHelpers
{
	...
	public static class StatementsMatchingProto
	{
		...
		{
		    return CompareRefNbr(graph, aDetail, aTran.ExtRefNbr, looseCompare);
		}

	    public static decimal CompareRefNbr(CABankTransactionsMaint graph, CABankTran aDetail, string extRefNbr, bool looseCompare)
	    {
	        if (looseCompare)
	            return graph.EvaluateMatching(aDetail.ExtRefNbr, extRefNbr, false, graph.matchSettings.Current.EmptyRefNbrMatching ?? true);
	        else
	            return graph.EvaluateTideMatching(aDetail.ExtRefNbr, extRefNbr, false, graph.matchSettings.Current.EmptyRefNbrMatching ?? true);
	    }
	    ...
	    public static decimal EvaluateMatching(CABankTransactionsMaint graph, CABankTran aDetail, CABankTranExpenseDetailMatch expenseMath, MatchSettings aSettings)
        {
	        decimal relevance = Decimal.Zero;
	        ...
	        relevance += graph.CompareRefNbr(aDetail, expenseMath, looseCompare) * weights[1];
