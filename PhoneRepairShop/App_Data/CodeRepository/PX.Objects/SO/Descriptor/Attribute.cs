...
namespace PX.Objects.SO
{
        ...
        where CustomerField: class,IBqlField
	{
		...
		{
			if (VerifyEnabled && (!IsLongOperationProcessing(sender.Graph) || VerifyOnLongRun(sender, Row, e)))
		...
		{
			return SOOrderType.PK.Find(sender.Graph, ((SOOrder)Row).OrderType)?.CreditHoldEntry ?? false;
		}
	...
	public class SOUnbilledTaxAttribute : SOTaxAttribute
	{
			...
			decimal CuryTaxDiscountTotal)
		{
			...
			decimal CuryUnbilledDiscTotal = (Math.Abs(CuryLineTotal - CuryUnbilledLineTotal) < 0.00005m)
				? CuryDiscTotal
