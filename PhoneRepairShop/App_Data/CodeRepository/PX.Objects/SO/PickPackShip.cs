...
namespace PX.Objects.SO
{
	...
	public class PickPackShip : WMSBase
	{
		...
		protected override void _(Events.RowSelected<Header> e)
		{
			...
			bool isNotConfirmed = e.Row.Mode.IsIn(Modes.PickWave, Modes.PickBatch)
				? SOPicker.PK.Find(Base, e.Row.WorksheetNbr, e.Row.PickerNbr)?.Confirmed == false
				: e.Row.RefNbr.With(nbr => SOShipment.PK.Find(Base, nbr)?.Confirmed) == false;
			...
			bool hasNotConfirmedLines = isNotConfirmed && (e.Row.Remove == true || CanPick || CanPickWS || HasUnconfirmedBoxes && e.Row.PackageLineNbr != null);
		...
		protected virtual SOShipment Shipment => Base.CurrentDocument.Current;
		...
		protected virtual bool ConfirmPicked()
		{
			...
			if (!isSerialItem && noQtyState && Info.Current.MessageType == WMSMessageTypes.Information)
				HeaderSetter.Set(h => h.IsQtyOverridable, true);
			return true;
		}
		...
		protected virtual SOPickerListEntry GetSelectedPickListEntry(Header header)
		{
			...
			var pickedSplit = PickListOfPicker
				...
				.OrderByDescending(r => r.Split.IsUnassigned == false)
		...
		protected virtual void ConfirmShipment(bool confirmAll)
		{
			...
			if (HeaderView.Current.Mode.IsIn(Modes.Pack, Modes.Ship))
		...
		protected override void OnWaitEnd(PXLongRunStatus status, SOShipment primaryRow)
		{
			if (HeaderView.Current.Mode.IsIn(Modes.PickWave, Modes.PickBatch))
			{
				var picker = SOPicker.PK.Find(Base, HeaderView.Current.WorksheetNbr, HeaderView.Current.PickerNbr);
				if (picker?.Confirmed == true)
				{
					if (Base.IsMobile)
						Clear(Msg.PickListIsConfirmed);
					else
						SetScanState(GetDefaultState(), Msg.PickListIsConfirmed);
				}
				else if (status == PXLongRunStatus.Aborted)
				{
					SetScanState(ScanStates.Command, Msg.PickListConfirmationFailed);
				}
			}
			else
			{
				OnWaitEnd(status, primaryRow?.Confirmed == true, Msg.ShipmentIsConfirmed, Msg.ShipmentConfirmationFailed);
			}
		}
		...
		protected virtual bool DocumentIsConfirmed => HeaderView.Current.Mode.IsIn(Modes.PickWave, Modes.PickBatch) && Picker.Current?.Confirmed == true || Shipment?.Confirmed == true;
