...
namespace PX.Objects.FS
{
        ...
        where TPostLine : class, PX.Data.IBqlTable, IPostLine, new()
    {
        ...
        public virtual void CreateInvoiceDocument(MethodParm parm)
        {
            ...
            List<DocLineExt> docLines = parm.MyStep.docLines.OrderBy(x => x.docLine.SrvOrdType).ThenBy(x => x.docLine.RefNbr).ThenBy(x => x.docLine.TabOrigin).ThenBy(x => x.docLine.SortOrder).ToList();
            List<DocLineExt> docLinesGrouped = docLines.GroupBy(x => new { x.docLine.DocID, x.docLine.LineID }).Select(g => g.First()).OrderBy(x => x.docLine.SrvOrdType).ThenBy(x => x.docLine.RefNbr).ThenBy(x => x.docLine.TabOrigin).ThenBy(x => x.docLine.SortOrder).ToList();
        ...
        public static void CreateInvoiceDocumentCheckResult(MethodParm parm)
        {
            ...
            else
            {
                ...
                var warningException_ServiceOrder = new PXException(
                        PXMessages.LocalizeFormatNoPrefix(TX.Messages.ERRORS_IN_OTHER_SERVICEORDER));
                var warningException_Appointment = new PXException(
                        PXMessages.LocalizeFormatNoPrefix(TX.Messages.ERRORS_IN_OTHER_APPOINTMENT));
                            ...
                            else
                            {
                                ...
                                if (errorMsgBuilder.Length > 0)
                                {
                                    ...
                                    exception = new PXException(
                                        exceptionMessage + " "
                                        + errorMsgBuilder.ToString().TrimEnd());
                                }
                                else
                                {
                                    showAsError = false;
