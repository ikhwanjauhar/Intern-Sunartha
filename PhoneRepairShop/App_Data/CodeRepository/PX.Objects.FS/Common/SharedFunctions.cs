...
namespace PX.Objects.FS
{
    public static class SharedFunctions
    {
            ...
            where costCodeField : IBqlField
        {
            ...
            bool enableField = lineType != ID.LineType_ServiceContract.COMMENT_SERVICE
                                && lineType != ID.LineType_ServiceContract.COMMENT_PART
                                && lineType != ID.LineType_ServiceContract.INSTRUCTION_SERVICE
                                && lineType != ID.LineType_ServiceContract.INSTRUCTION_PART;
        ...
        public static void ValidateDuplicateLineNbr(
                                PXSelectBase<FSSODetService> soServices,
                                PXSelectBase<FSSODetPart> soParts,
                                PXSelectBase<FSAppointmentDetService> apptServices,
                                PXSelectBase<FSAppointmentDetPart> apptParts,
                                PXSelectBase<FSAppointmentInventoryItem> apptPickupDelivery)
        {
            var lineNbrs = new List<int?>();

            if (soServices != null)
            {
                ValidateDuplicateLineNbr<FSSODetService>(lineNbrs,
                    soServices.Select().ToList().Select(e => (FSSODetService)e).ToList(),
                    soServices.Cache);
            }

            if (soParts != null)
            {
                ValidateDuplicateLineNbr<FSSODetPart>(lineNbrs,
                    soParts.Select().ToList().Select(e => (FSSODetPart)e).ToList(),
                    soParts.Cache);
            }

            if (apptServices != null)
            {
                ValidateDuplicateLineNbr<FSAppointmentDetService>(lineNbrs,
                    apptServices.Select().ToList().Select(e => (FSAppointmentDetService)e).ToList(),
                    apptServices.Cache);
            }

            if (apptParts != null)
            {
                ValidateDuplicateLineNbr<FSAppointmentDetPart>(lineNbrs,
                    apptParts.Select().ToList().Select(e => (FSAppointmentDetPart)e).ToList(),
                    apptParts.Cache);
            }

            if (apptPickupDelivery != null)
            {
                ValidateDuplicateLineNbr<FSAppointmentInventoryItem>(lineNbrs,
                    apptPickupDelivery.Select().ToList().Select(e => (FSAppointmentInventoryItem)e).ToList(),
                    apptPickupDelivery.Cache);
            }
        }
