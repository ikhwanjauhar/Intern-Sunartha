...
namespace PX.Objects.FS
{
    ...
    public class FSSelectorSODetIDServiceAttribute : PXSelectorAttribute
    {
        public FSSelectorSODetIDServiceAttribute()
            : base(
                typeof(Search<FSSODetService.sODetID,
                       Where<
                           FSSODetService.sOID, Equal<Current<FSAppointment.sOID>>,
                           And<FSSODetService.status, NotEqual<ListField_Status_AppointmentDet.Canceled>,
                           And<FSSODetService.status, NotEqual<ListField_Status_AppointmentDet.Completed>>>>>),
                new Type[]
                {
                    typeof(FSSODetService.lineRef), 
                    typeof(FSSODetService.lineType),
                    typeof(FSSODetService.status), 
                    typeof(FSSODetService.inventoryID), 
                    typeof(FSSODetService.lastModifiedDateTime)
                })
        {
            SubstituteKey = typeof(FSSODetService.lineRef);
        }
    }
    #endregion
    ...
    public class FSSelectorServiceOrderSODetIDAttribute : PXSelectorAttribute
    {
        public FSSelectorServiceOrderSODetIDAttribute()
            : base(
                typeof(Search<FSSODetService.lineRef,
                            Where<FSSODetService.sOID, Equal<Current<FSServiceOrder.sOID>>,
                                And<FSSODetService.lineType, Equal<ListField_LineType_ALL.Service>>>>),
                new Type[]
                {
                    typeof(FSSODetService.lineRef),
                    typeof(FSSODetService.lineType),
                    typeof(FSSODetService.status),
                    typeof(FSSODetService.inventoryID),
                    typeof(FSSODetService.tranDesc)
                })
        {
            SubstituteKey = typeof(FSSODetService.lineRef);
            DescriptionField = typeof(FSSODetService.inventoryID);
            DirtyRead = true;
        }
    }
    ...
    public class FSSelectorAppointmentSODetIDAttribute : PXSelectorAttribute
    {
        public FSSelectorAppointmentSODetIDAttribute()
            : base(
                typeof(Search<FSAppointmentDetService.lineRef,
                            Where<
                                FSAppointmentDetService.appointmentID, Equal<Current<FSAppointment.appointmentID>>,
                                And<FSAppointmentDetService.lineType, Equal<ListField_LineType_ALL.Service>,
                                And<FSAppointmentDetService.lineRef, IsNotNull>>>>),
                new Type[]
                {
                    typeof(FSAppointmentDetService.lineRef),
                    typeof(FSAppointmentDetService.lineType),
                    typeof(FSAppointmentDetService.status),
                    typeof(FSAppointmentDetService.inventoryID),
                    typeof(FSAppointmentDetService.tranDesc)
                })
        {
            SubstituteKey = typeof(FSAppointmentDetService.lineRef);
            DescriptionField = typeof(FSAppointmentDetService.inventoryID);
            DirtyRead = true;
        }
    }
    #endregion
        ...
        public FSSelectorServiceSODetIDInAppointmentAttribute()
            : base(
                typeof(Search2<FSSODet.sODetID,
                            InnerJoin<FSAppointmentDetService, 
                                On<FSAppointmentDetService.sODetID, Equal<FSSODet.sODetID>>,
                            InnerJoin<FSAppointment,
                                On<FSAppointment.appointmentID, Equal<FSAppointmentDetService.appointmentID>>>>,
                        Where<
                            ...
                            And<FSAppointmentDetService.lineType, Equal<FSAppointmentDetService.lineType.Service>,
                            And<
                                Where<
                                    FSAppointmentDetService.serviceType, Equal<FSAppointmentDetService.serviceType.Delivered_Items>,
                                    Or<FSAppointmentDetService.serviceType, Equal<FSAppointmentDetService.serviceType.Picked_Up_Items>>>>>>>>>>>),
                new Type[]
                {
    ...
    public class FSSelectorSODetIDPartAttribute : PXSelectorAttribute
    {
        public FSSelectorSODetIDPartAttribute()
            : base(
                typeof(Search<FSSODetPart.sODetID,
                       Where<
                           FSSODetPart.sOID, Equal<Current<FSAppointment.sOID>>>>),
                new Type[]
                {
                    typeof(FSSODetPart.lineRef), 
                    typeof(FSSODetPart.lineType), 
                    typeof(FSSODetPart.inventoryID),
                    typeof(FSSODetPart.lastModifiedDateTime)
                })
        {
            SubstituteKey = typeof(FSSODetPart.lineRef);
        }
    }
    #endregion
