...
namespace PX.Objects.FS
{
    public static class AppointmentCore
    {
        ...
        public class AppointmentDetServices_View : PXOrderedSelect<FSAppointment, FSAppointmentDetService,
                LeftJoin<FSSODet,
                ...
                OrderBy<
                    Asc<FSAppointmentDetService.srvOrdType,
                    Asc<FSAppointmentDetService.refNbr,
                    Asc<FSAppointmentDetService.sortOrder,
                    Asc<FSAppointmentDetService.lineNbr>>>>>>
        {
        ...
        [PXDynamicButton(new string[] { PartPasteLineCommand, PartResetOrderCommand },
                         new string[] { PX.Data.ActionsMessages.PasteLine, PX.Data.ActionsMessages.ResetOrder },
                         TranslationKeyType = typeof(PX.Objects.Common.Messages))]
        public class AppointmentDetParts_View : PXOrderedSelect<FSAppointment, FSAppointmentDetPart,
                LeftJoin<FSSODet,
                ...
                OrderBy<
                    Asc<FSAppointmentDetPart.srvOrdType,
                    Asc<FSAppointmentDetPart.refNbr,
                    Asc<FSAppointmentDetPart.sortOrder,
                    Asc<FSAppointmentDetPart.lineNbr>>>>>>
        {
                                                        ...
                                                        Where<
                                                                ...
                                                                And<
                                                                    Where<FSAppointmentEmployee.serviceLineRef, IsNull,
                                                                        Or<FSAppointmentServiceEmployee.lineType, Equal<FSAppointmentDet.lineType.Service>>>>>,
                                                        OrderBy<
        ...
        public class AppointmentPickupDeliveryItems_View : PXSelectJoin<FSAppointmentInventoryItem,
                                                            ...
                                                            Where<FSAppointmentInventoryItem.appointmentID, Equal<Current<FSAppointment.appointmentID>>>>
        {
        ...
        public static void UpdateStaffRelatedUnboundFields(FSAppointmentDet fsAppointmentDetServiceRow, AppointmentServiceEmployees_View appointmentEmployees, int? numEmployeeLinkedToService = null)
        {
        ...
        public static void InsertUpdateDelete_AppointmentDetService_StaffID(PXCache cache, FSAppointmentDet fsAppointmentDetServiceRow, AppointmentServiceEmployees_View appointmentEmployees, int? oldStaffID)
        {
        ...
        public static void VerifyStatusTransition(PXGraph graph,
                                                  FSAppointment fsAppointmentRow,
                                                  FSServiceOrder fsServiceOrderRow,
                                                  string newAppointmentStatus)
        {
            ...
            if (AppointmentCore.CheckNewAppointmentStatus(graph, fsAppointmentRow, fsServiceOrderRow, newAppointmentStatus, ref errorMessage) == false)
            {
        ...
        public static void InsertUpdateAppointmentInfoInServiceOrder<SODetType>(
                    PXCache appCache,
                    FSAppointment fsAppointmentRow,
                    PXCache detCache,
                    FSAppointmentDet fsAppointmentDetRow,
                    ServiceOrderCore.ServiceOrderRecords_View viewServiceOrderRelated,
            ...
            where SODetType : FSSODet, new()
        {
            ...
            decimal? _CuryTranAmt = 0m;
            decimal? _ApptQty = 0m;
            int? _ApptDuration = 0;
            int? _ApptNumber = 0;

            //Actual fields must always be zero for these conditions:
            bool zeroActualField = apptNotStarted == true 
                                    || fsAppointmentDetRow.Status == ID.Status_AppointmentDet.CANCELED;

            decimal? _OriCuryTranAmt = (decimal?)detCache.GetValueOriginal<FSAppointmentDet.curyTranAmt>(fsAppointmentDetRow) ?? 0;
            if (_OriCuryTranAmt != fsAppointmentDetRow.CuryTranAmt && zeroActualField == false)
            {
                _CuryTranAmt = fsAppointmentDetRow.CuryTranAmt - (_OriCuryTranAmt ?? 0);
            }

            decimal? _OriApptQty = (decimal?)detCache.GetValueOriginal<FSAppointmentDet.qty>(fsAppointmentDetRow) ?? 0;
            if (_OriApptQty != fsAppointmentDetRow.Qty && zeroActualField == false)
            {
                _ApptQty = fsAppointmentDetRow.Qty - (_OriApptQty ?? 0);
            }

            int? _OriApptDuration = (int?)detCache.GetValueOriginal<FSAppointmentDet.actualDuration>(fsAppointmentDetRow) ?? 0;
            if (_OriApptDuration != fsAppointmentDetRow.ActualDuration && zeroActualField == false)
            {
            ...
            if (fsSODetRow != null)
            {
                ...
                switch (detStatus)
                {
                    ...
                    case PXEntryStatus.Updated:
                        ...
                        {
                            _CuryTranAmt = fsAppointmentDetRow.CuryTranAmt;
                            _ApptQty = fsAppointmentDetRow.Qty;
                ...
                {
                    decimal? curyApptTranAmt = fsSODetRow.CuryApptTranAmt + _CuryTranAmt;
                    decimal? apptQty = fsSODetRow.ApptQty + _ApptQty;
                    int? apptDuration = fsSODetRow.ApptDuration + _ApptDuration;
                    int? apptNumber = fsSODetRow.ApptNumber + _ApptNumber;
                    ...
                    fsSODetRow.ApptNumber = apptNumber;
        ...
        public static void DeleteAppointmentInfoInServiceOrder(
                    PXCache detCache,
                    FSAppointmentDet fsAppointmentDetRow,
        ...
        public static void FSAppointmentDet_CuryBillable_FieldUpdating(PXCache cacheAppointmentDet,
                                                                        PXCache cacheAppointment,
                                                                        FSAppointmentDet fsAppointmentDetRow,
                                                                        FSAppointment fsAppointmentRow,
