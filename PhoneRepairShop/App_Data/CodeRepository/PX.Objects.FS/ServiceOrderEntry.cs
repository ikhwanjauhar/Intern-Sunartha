...
namespace PX.Objects.FS
{
    public class ServiceOrderEntry : PXGraph<ServiceOrderEntry, FSServiceOrder>, IGraphWithInitialization
    {
        ...
        public bool SkipFSSODetUnionUpdate = false;
        ...
        public ServiceOrderCore.ServiceOrderDetServicesOrdered ServiceOrderDetServices;
        
        [PXImport(typeof(FSServiceOrder))]
        [PXViewName(TX.FriendlyViewName.ServiceOrder.SERVICEORDER_DET_PARTS)]
        public ServiceOrderCore.ServiceOrderDetPartsOrdered ServiceOrderDetParts;
        ...
        private FSSODetUNION CreateTaxRow(FSSODet baseRow)
        {
            var taxRow = new FSSODetUNION()
            {
                ...
                BranchID = baseRow.BranchID,
                LineType = baseRow.LineType,
                InventoryID = baseRow.InventoryID,
                SiteID = baseRow.SiteID,
                BillCustomerID = baseRow.BillCustomerID,
                VendorID = baseRow.VendorID,
                EstimatedQty = baseRow.EstimatedQty,
                CuryUnitPrice = baseRow.CuryUnitPrice,

                GroupDiscountRate = baseRow.GroupDiscountRate,
                DocumentDiscountRate = baseRow.DocumentDiscountRate,

                CuryDiscAmt = baseRow.CuryDiscAmt,
                DiscPct = baseRow.DiscPct,
                DiscountID = baseRow.DiscountID,
                DiscountSequenceID = baseRow.DiscountSequenceID,
                IsFree = baseRow.IsFree,
                ManualDisc = baseRow.ManualDisc,
                ManualPrice = baseRow.ManualPrice,

                CuryInfoID = baseRow.CuryInfoID,
                TaxCategoryID = baseRow.TaxCategoryID,
                CuryEstimatedTranAmt = baseRow.CuryEstimatedTranAmt,
                CuryBillableExtPrice = baseRow.CuryBillableExtPrice,
        ...
        public virtual IEnumerable CompleteOrder(PXAdapter adapter)
        {
            ...
            foreach (FSServiceOrder fsServiceOrderRow in list)
            {
                ServiceOrderCore.ChangeStatus_Handler(this, fsServiceOrderRow, BillingCycleRelated.Current, ID.Status_ServiceOrder.COMPLETED);
            }
        ...
        /// </summary>
        public virtual void EnableDisableServiceDetTabs(FSServiceOrder fsServiceOrderRow)
        {
        ...
        public virtual void EnableDisable_StaffID(PXCache cache, FSSODetService fsSODetServiceRow)
        {
            bool enableStaffID = fsSODetServiceRow.EnableStaffID == true
                                      && fsSODetServiceRow.LineType == ID.LineType_All.SERVICE
                                      && fsSODetServiceRow.InventoryID != null;
        ...
        public virtual void SetSODetServiceLastReferencedBy(FSSODet fsSODetRow)
        {
            ...
            if (LastRefAppointmentDetails == null)
            {
                LastRefAppointmentDetails = PXSelectJoinGroupBy<
                            ...
                            Aggregate<
                                GroupBy<FSAppointmentDet.sODetID>>>.Select(this).ToList();
            }
        ...
        public virtual void HideOrShowTabs(FSServiceOrder fsServiceOrderRow)
        {
            ...
            PXUIFieldAttribute.SetVisible<FSSODetService.staffID>(ServiceOrderDetServices.Cache, 
                                                                  ServiceOrderDetServices.Current, 
        ...
        public virtual void SetDefaultVendorSettingsForItem(PXCache cache, ServiceOrderEntry serviceOrderEntry, FSSODetPart fsSODetPartRow)
        {
            ...
            if (e.TranStatus == PXTranStatus.Open)
            {
                ...
                if (fsServiceOrderRow.UpdateAppWaitingForParts == true)
                {
                    ...
                    else
                    {
                        ...
                        foreach (FSSODet temp in ServiceOrderDetParts.Select().RowCast<FSSODetPart>().Where(_ => _.EnablePO == true && _.POCompleted != true))
                        {
                            soDetList.Add(temp);
                        }

                        foreach (FSSODet temp in ServiceOrderDetServices.Select().RowCast<FSSODetService>().Where(_ => _.EnablePO == true && _.POCompleted != true))
                        {
                            soDetList.Add(temp);
                        }
        ...
        protected virtual void FSSODetService_BillableQty_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
        {
        ...
        protected virtual void FSSODetService_InventoryID_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
        {
            ...
            if (fsSODetServiceRow.ContractRelated == true)
            {
                ...
                cache.SetValueExt<FSSODetService.projectTaskID>(fsSODetServiceRow, fsContractPeriodDetRow.ProjectTaskID);
                cache.SetValueExt<FSSODetService.costCodeID>(fsSODetServiceRow, fsContractPeriodDetRow.CostCodeID);
            }
		...
		protected virtual void FSSODetService_RowSelecting(PXCache cache, PXRowSelectingEventArgs e)
		{
            ...
            FSSODetService fsSODetServiceRow = (FSSODetService)e.Row;
            fsSODetServiceRow.IsFree = ServiceOrderAppointmentHandlers.IsFree(fsSODetServiceRow.BillingRule, fsSODetServiceRow.ManualPrice, fsSODetServiceRow.LineType);
        ...
        protected virtual void FSSODetService_RowUpdated(PXCache cache, PXRowUpdatedEventArgs e)
        {
            ...
            if (SkipFSSODetUnionUpdate == false)
            {
        ...
        protected virtual void FSSODetService_SiteID_FieldDefaulting(PXCache cache, PXFieldDefaultingEventArgs e)
        {
            if (e.Row == null)
            {
                return;
            ...
            if (ServiceOrderAppointmentHandlers.IsInventoryLine(fsSODetServiceRow.LineType) == false
                || fsSODetServiceRow.InventoryID == null)
            {
                e.NewValue = null;
                e.Cancel = true;
        ...
        protected virtual void FSSODetPart_SiteID_FieldDefaulting(PXCache cache, PXFieldDefaultingEventArgs e)
        {
            if (e.Row == null)
            {
                return;
            ...
            FSSODetPart fsFSSODetPartRow = (FSSODetPart)e.Row;

            if (ServiceOrderAppointmentHandlers.IsInventoryLine(fsFSSODetPartRow.LineType) == false
                || fsFSSODetPartRow.InventoryID == null)
            {
                e.NewValue = null;
                e.Cancel = true;
            }
        ...
        protected virtual void FSSODetPart_RowSelecting(PXCache cache, PXRowSelectingEventArgs e)
        {
            ...
            FSSODetPart fsSODetPartRow = (FSSODetPart)e.Row;
            fsSODetPartRow.IsFree = ServiceOrderAppointmentHandlers.IsFree(fsSODetPartRow.BillingRule, fsSODetPartRow.ManualPrice, fsSODetPartRow.LineType);

            if (GraphAppointmentEntryCaller == null)
            {
                using (new PXConnectionScope())
                {
                    SetSODetServiceLastReferencedBy(fsSODetPartRow);
                }
            }
        }
        ...
        protected virtual void FSSODetPart_BillableQty_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
        {
        ...
        protected virtual void FSSODetService_CostCodeID_FieldDefaulting(PXCache cache, PXFieldDefaultingEventArgs e)
        {
            if (e.Row == null)
            {
                return;
            ...
            ServiceOrderCore.SetCostCodeDefault(fsSODetServiceRow, CurrentServiceOrder.Current?.ProjectID, ServiceOrderTypeSelected.Current, e);
        }

        protected virtual void FSSODetPart_CostCodeID_FieldDefaulting(PXCache cache, PXFieldDefaultingEventArgs e)
        {
            if (e.Row == null)
            {
                return;
            ...
            ServiceOrderCore.SetCostCodeDefault(fsSODetPartRow, CurrentServiceOrder.Current?.ProjectID, ServiceOrderTypeSelected.Current, e);
        }

        protected virtual void FSSODetService_ContractRelated_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
        {
            if (e.Row == null)
            {
                return;
            ...
            if ((bool?)e.OldValue != ((FSSODetService)e.Row).ContractRelated)
            {
                cache.SetDefaultExt<FSSODetService.curyUnitPrice>(e.Row);
            }
        }

        protected virtual void FSSODetPart_ContractRelated_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
        {
            if (e.Row == null)
            {
                return;
            ...
            if ((bool?)e.OldValue != ((FSSODetPart)e.Row).ContractRelated)
            {
                cache.SetDefaultExt<FSSODetPart.curyUnitPrice>(e.Row);
            }
        }
        #endregion
        ...
        protected virtual void BAccountStaffMember_RowUpdated(PXCache cache, PXRowUpdatedEventArgs e)
        {
            ...
            if (StaffSelectorFilter.Current != null)
            {
                ...
                {
                    if (ServiceOrderDetServices.Current.LineRef != StaffSelectorFilter.Current.ServiceLineRef)
                    {
                        ServiceOrderDetServices.Current = ServiceOrderDetServices.Search<FSSODetService.lineRef>(StaffSelectorFilter.Current.ServiceLineRef);
                    }
                    ...
                    {
                        ServiceOrderDetServices.Current.StaffID = bAccountStaffMemberRow.BAccountID;
                    }
                    else
                    {
                        ServiceOrderDetServices.Current.StaffID = null;
                    }
        ...
        public class SalesTax : TaxGraph<ServiceOrderEntry, FSServiceOrder>
        {
            ...
            protected override DetailMapping GetDetailMapping()
            {
                return new DetailMapping(typeof(FSSODetUNION))
                {
                    ...
                    CuryTranDiscount = typeof(FSSODetUNION.curyDiscAmt),
                    CuryTranExtPrice = typeof(FSSODetUNION.curyBillableExtPrice)
                };
            }
