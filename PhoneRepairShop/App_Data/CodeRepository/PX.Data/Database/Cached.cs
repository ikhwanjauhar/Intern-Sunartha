			...
			foreach (PXDataRecord r in result)
			{
				...
				for (int i = 0; i < caches.Length; i++)
				{
					...
					if (_LegacyQueryCacheMode)
						...
						items[i] = caches[i].CreateItem(rec, ref position, _IsReadOnly || i > 0);
					if (wasUpdated)
					{
						overrideSort = true;
						extFilter = true;
						...
						for (int i = 0; i < list.Count; i++)
						{
							...
							else
							{
								...
								else
								{
									...
									CloneOriginals(caches[0], item, clone);
									result[i] = clone;
									result[i] = caches[0].Select(result[i], _IsReadOnly, out wasUpdated); ;
									if (wasUpdated)
									{
										overrideSort = true;
										needFilterResults = true;
...
//			IBqlParameter[] selpars = _Select.GetParameters();
        ...
        where OrderBy : IBqlOrderBy, new()
    {
        ...
        {
            this.View = new PXView(graph, false, GetCommand().OrderByNew(typeof(OrderBy)), (PXSelectDelegate)Getter) { CacheType = typeof(TTable) };
        ...
        where TTable : class, IBqlTable, new()
    {
        ...
        public PXNonSqlView(PXGraph graph) : base(graph)
        {
            ...
            {
                this.Cache.Interceptor = new Interceptor
                {
